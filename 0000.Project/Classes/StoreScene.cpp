...
bool StoreScene::init()
{
	...
	CCSprite* title = CCSprite::create("Images/Scene/Store_bg.png");
	...
	tableView1->setDirection(ScrollView::Direction::VERTICAL);
	//tableView1->setDirection(ScrollView::Direction::HORIZONTAL);
	tableView1->setPosition(Vec2(630 + (winSize.width - 1230) / 2, (winSize.height - 400) / 2));
	tableView1->setAnchorPoint(Vec2(0, 0));
	tableView1->setDelegate(this);
	tableView1->setTag(200);
	//tableView1->setBounceable(false);
	tableView1->setVerticalFillOrder(TableView::VerticalFillOrder::TOP_DOWN);
	tableView1->reloadData();
	this->addChild(tableView1);
...
//	auto pMenuItem2 = MenuItemFont::create("진화", CC_CALLBACK_1(StoreScene::doContinue, this));
//	pMenuItem2->setColor(Color3B(255, 255, 255));
//	/*
//	auto pMenuItem3 = MenuItemFont::create("방출", CC_CALLBACK_1(StoreScene::doClose, this));
...
void StoreScene::UpdateMonsterDB(int num, int Type) {
	sqlite3* pDB = nullptr;
	char* errMsg = nullptr;
	int result;
...
void StoreScene::doContinue(Ref* pSender) {
	if (Monster_List[MonsterCellNum].level != 10) {
		...
		if (CheckClass(Monster_List[MonsterCellNum].Type)) {
			...
			auto cbSound = CallFunc::create(CC_CALLBACK_0(StoreScene::RemoveSprite, this));
...
bool StoreScene::CheckClass(int num) {
...
void StoreScene::doClose(Ref* pSender) {
...
void StoreScene::Monster(int num, int row) {
	...
	if (num == 9) {
		Items = Sprite::createWithSpriteFrameName("items9.png");
	}
...
void StoreScene::MonsterAdd(int num, Sprite *st) {
	//땅질퍽이
	char str1[100];
	char str2[100];
