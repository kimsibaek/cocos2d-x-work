...
bool ThirdScene::init()
{
	...
	tmap = TMXTiledMap::create("Images/TileMapSocoban3.tmx");
	...
	//버튼
	auto Right_button = MenuItemImage::create("Images/arrow1off.png", "Images/arrow1on.png", CC_CALLBACK_1(ThirdScene::Right_button_Action, this));
	auto pMenu = Menu::create(Right_button, nullptr);
	pMenu->setPosition(430, 70);
	this->addChild(pMenu);

	auto Left_button = MenuItemImage::create("Images/arrow3off.png", "Images/arrow3on.png", CC_CALLBACK_1(ThirdScene::Left_button_Action, this));
	auto pMenu1 = Menu::create(Left_button, nullptr);
	pMenu1->setPosition(350, 70);
	this->addChild(pMenu1);

	auto Up_button = MenuItemImage::create("Images/arrow4off.png", "Images/arrow4on.png", CC_CALLBACK_1(ThirdScene::Up_button_Action, this));
	auto pMenu2 = Menu::create(Up_button, nullptr);
	pMenu2->setPosition(390, 110);
	this->addChild(pMenu2);

	auto Down_button = MenuItemImage::create("Images/arrow2off.png", "Images/arrow2on.png", CC_CALLBACK_1(ThirdScene::Down_button_Action, this));
	auto pMenu3 = Menu::create(Down_button, nullptr);
	pMenu3->setPosition(390, 30);
	this->addChild(pMenu3);

	auto Back_button = MenuItemImage::create("Images/back_off.png", "Images/back_on.png", CC_CALLBACK_1(ThirdScene::Back_button_Action, this));
	auto pMenu4 = Menu::create(Back_button, nullptr);
	pMenu4->setPosition(120, 50);
	this->addChild(pMenu4);

	auto Retset_button = MenuItemImage::create("Images/restart_off.png", "Images/restart_on.png", CC_CALLBACK_1(ThirdScene::Retset_button_Action, this));
...
void ThirdScene::createPenguin(Vec2 vector) {
	//man
	texture = Director::getInstance()->getTextureCache()->addImage("Images/game.png");
...
Vec2 ThirdScene::tileCoordForPosition(cocos2d::Vec2 position) {
	int x = position.x / tmap->getTileSize().width;
	int y = ((tmap->getMapSize().height * tmap->getTileSize().height) - position.y) / tmap->getTileSize().height;
	return Vec2(x, y);
}

bool ThirdScene::setPlayerPosition(Vec2 position, int num) {
	int ice_num = 100;
	Vec2 After2Postion;
	Vec2 IcePostion;
	if (num == 1) {
...
bool ThirdScene::setIcePosition(Vec2 position, int num) {
	Vec2 IcePostion;
	if (num == 1) {
...
void ThirdScene::Right_button_Action(Ref* pSender) {
...
void ThirdScene::Left_button_Action(Ref* pSender) {
	Vec2 playerPos = Penguin->getPosition();
	AfterPosition = playerPos + Vec2(-20, 0);
...
void ThirdScene::Up_button_Action(Ref* pSender) {
	Vec2 playerPos = Penguin->getPosition();
...
void ThirdScene::Down_button_Action(Ref* pSender) {
	Vec2 playerPos = Penguin->getPosition();
	AfterPosition = playerPos + Vec2(0, -20);
...
void ThirdScene::Back_button_Action(Ref* pSender) {
...
void ThirdScene::Retset_button_Action(Ref* pSender) {
	auto pScene = ThirdScene::createScene();
	Director::getInstance()->replaceScene(pScene);
}

void ThirdScene::FinishCheck() {
	int num = 0;
