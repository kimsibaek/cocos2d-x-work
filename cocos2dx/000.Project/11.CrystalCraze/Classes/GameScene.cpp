...
bool GameScene::init()
{
	...
	gBoardSprites = cocos2d::Vector<cocos2d::Sprite*>(kNumTotalGems);
	...
	//update
	this->schedule(schedule_selector(GameScene::onUpdate), 0.4f);
...
void GameScene::setGemType(int x, int y, int newType)
{
	...
	//보류 :: sprite 교체
	//gBoardSprites.at(idx) = gemSprite;
...
void GameScene::onUpdate(float f) {
	if (!gIsGameOver)
	{
		...
		{
			if (gNumGemsInColumn[x] + gFallingGemsSize[x] < kBoardHeight && gTimeSinceAddInColumn[x] >= kTimeBetweenGemAdds)
			{
				...
				gFallingGems[x].sprite_num[gFallingGemsSize[x]] = gem;
				gFallingGemsSize[x]++;
				log("gFallingGemsSize[%d] = %d", x, gFallingGemsSize[x]);
				this->addChild(gemSprite);
		...
		for (x = 0; x < kBoardWidth; x++)
		{
			...
			for (int i = numFallingGems - 1; i >= 0; i--)
			{
				...
				if (gem.yPos <= gNumGemsInColumn[x])
				{
					...
					{
						char wav[25];
						sprintf(wav, "sounds/%d.wav", rand() * 4);
						m_nSoundId = SimpleAudioEngine::getInstance()->playEffect(wav);
						gemLanded = true;
					}
					if (gFallingGemsSize[x]) {
						column = (Sprite_num*)realloc(column, sizeof(Sprite_num) * (gFallingGemsSize[x] - 1));
						gFallingGemsSize[x]--;
					}
					// Insert into board
					...
					//sprite 교체
					char png[25];
					sprintf(png, "Images/%d.png", gem.gemType);
					gBoardSprites.at(x + y*kBoardWidth)->setTexture(Director::getInstance()->getTextureCache()->addImage(png));
